...
def compute_zone_stats(zones: dict[str, gpd.GeoDataFrame], class_lookup: dict) -> pd.DataFrame:
    path2000 = PROC / "classified_y2000.tif"
    path2025 = PROC / "classified_y2025.tif"
    rows = []
    with rasterio.open(path2000) as d0, rasterio.open(path2025) as d1:
        a0 = d0.read(1)
        a1 = d1.read(1)
...
def pick_sites(zones: dict, class_lookup: dict) -> gpd.GeoDataFrame:
    change_path = PROC / "change_theme_y2000_y2025.tif"
    class2025 = PROC / "classified_y2025.tif"
    class2000 = PROC / "classified_y2000.tif"
...
def plot_site_panel(site: pd.Series, out_path: Path) -> None:
    ...
    rgb00, _, ext = read_window(COMP / "composite_y2000.tif", bounds)
    rgb25, _, _ = read_window(COMP / "composite_y2025.tif", bounds)
    ch, _, _ = read_window(PROC / "change_theme_y2000_y2025.tif", bounds)
    ...
    axes[0].set_title("~2000 RGB")
    axes[1].imshow(rgb25, extent=ext, origin="upper")
    axes[1].set_title("~2025 RGB")
    im = axes[2].imshow(
...
def write_design_synthesis(zone_df: pd.DataFrame, sites: gpd.GeoDataFrame) -> None:
    ...
    lines = [
        ...
        "",
        f"- Built-up: **{ml['built_up_y2000_ha']:.1f} → {ml['built_up_y2025_ha']:.1f} ha** ({ml['built_up_change_ha']:+.1f} ha).",
        f"- Vegetation: **{ml['vegetation_y2000_ha']:.1f} → {ml['vegetation_y2025_ha']:.1f} ha** ({ml['vegetation_change_ha']:+.1f} ha).",
        f"- Open / bare: **{ml['open_bare_y2000_ha']:.1f} → {ml['open_bare_y2025_ha']:.1f} ha** ({ml['open_bare_change_ha']:+.1f} ha).",
        f"- Beach / sand: **{ml['beach_sand_y2000_ha']:.1f} → {ml['beach_sand_y2025_ha']:.1f} ha** ({ml['beach_sand_change_ha']:+.1f} ha).",
        "",
        ...
        "## Full 5 GN summary",
        "",
        f"- Built-up: **{gn5['built_up_y2000_ha']:.1f} → {gn5['built_up_y2025_ha']:.1f} ha** ({gn5['built_up_change_ha']:+.1f} ha).",
        f"- Vegetation: **{gn5['vegetation_y2000_ha']:.1f} → {gn5['vegetation_y2025_ha']:.1f} ha** ({gn5['vegetation_change_ha']:+.1f} ha).",
        f"- Open / bare: **{gn5['open_bare_y2000_ha']:.1f} → {gn5['open_bare_y2025_ha']:.1f} ha** ({gn5['open_bare_change_ha']:+.1f} ha).",
        "",
