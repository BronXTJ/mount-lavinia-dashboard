#!/usr/bin/env python3
"""Validate cul-de-sac Phase 3 walk-access overlay. Exit 0 only on PASS."""
...
METHODS = ROOT / "scripts" / "network_form" / "METHODS_CULDESAC_PHASE3.md"
...
def main() -> int:
    errors: list[str] = []
    hex_path = OUT / "culdesac_hex_walk.geojson"
    pts_path = OUT / "culdesacs_walk.geojson"
    sum_path = OUT / "culdesac_walk_summary.json"
    ...
    for f in feats:
        ...
        if not isinstance(n, int) or n < 1:
            ...
            continue
        total_n += n
        if p.get("walk_join_ok") is not True:
            fail(f"hex {p.get('hex_id')}: walk join missing", errors)
        if p.get("access_tier") is None:
            fail(f"hex {p.get('hex_id')}: access_tier null", errors)
    ...
    if len(pts) != EXPECTED_PRIMARY:
        fail(f"culdesacs_walk n {len(pts)} != {EXPECTED_PRIMARY}", errors)
    ...
    tier_rows = summary.get("by_access_tier") or []
    tier_sum = sum(int(r.get("n") or 0) for r in tier_rows)
    if tier_sum != assigned:
        fail(f"by_access_tier n sum {tier_sum} != assigned {assigned}", errors)

    if "desert" not in summary or "depth_x_tier" not in summary:
        fail("missing desert or depth_x_tier", errors)
    ...
    desert = summary.get("desert") or {}
    print("PASS")
    print(f"  hexes={len(feats)} assigned={assigned} outside={outside}")
    print(
        f"  desert_cul={desert.get('culdesac_n')} "
        f"mean_access={summary.get('mean_access_among_culdesac_hexes')}"
    )
    return 0
