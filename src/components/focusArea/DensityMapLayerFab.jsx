...
export default function DensityMapLayerFab({ visibleLayers, onToggle, basemapId, onBasemapChange }) {
  ...
  return (
    ...
    >
      ...
      {open && (
        <div
          className="density-fab-panel-enter w-[220px] overflow-hidden rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.6)]"
          ...
          <div className="py-1">
            {DENSITY_FAB_LAYERS.map((layer) => {
              ...
              return (
                <label
                  key={layer.id}
                  className="flex cursor-pointer items-center gap-2.5 px-4 py-2.5 hover:bg-white/[0.05]"
                >
                  <span
                    ...
                    aria-hidden="true"
                  />
                  <span className="min-w-0 flex-1 text-[13px] leading-snug" style={{ color: '#e0e0e0' }}>
                    {layer.label}
                  </span>
